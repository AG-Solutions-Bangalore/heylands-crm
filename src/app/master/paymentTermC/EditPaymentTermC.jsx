...
import { PaymentTermsCEdit } from "@/components/buttonIndex/ButtonComponents";
  ...
  const fetchStateData = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        paymentTermsC_status:
          paymentTermsCData.paymentTermsC_status || "Active",
      });
      setOriginalData({
        paymentTermsC: paymentTermsCData.paymentTermsC || "",
        paymentTermsC_status:
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          error.response?.data?.message || "Failed to update payment term c",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
              ...
              <div className="grid gap-1">
                ...
                <div className="relative">
                  <textarea
                    ...
                    className={
                      hasChanges
                        ? "w-full p-1 border-blue-200"
                        : "w-full p-1 border border-gray-300 rounded-sm"
                    }
                  />
                  {hasChanges &&
                    formData.paymentTermsC !== originalData.paymentTermsC && (
                      <div className="absolute right-2 top-1/2 -translate-y-1/2">
                        <RefreshCcw
                          ...
                          onClick={() =>
                            setFormData((prev) => ({
                              ...prev,
                              paymentTermsC: originalData.paymentTermsC,
                            }))
                          }
                        />
                      </div>
