...
const PurchaseProductList = () => {
  const {
    ...
    queryFn: async () => {
      ...
      const response = await axios.get(
        `${BASE_URL}/api/panel-fetch-purchase-order-product-list`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      return response.data.purchaseorderproduct;
    },
  });
  ...
  const columns = [
    ...
    {
      accessorKey: "purchaseOrderProduct",
      header: ({ column }) => (
        <Button
          variant="ghost"
...

    {
      ...
      header: "Status",
      cell: ({ row }) => {
        const status = row.getValue("purchaseOrderProduct_status");
        const purProdId = row.original.id;
        return (
          <StatusToggle
      ...
      <div className="w-full p-4">
        ...
        <div className="flex items-center py-4">
          ...
          <div className="relative w-72">
            ...
            <Input
              placeholder="Search purchase prouduct..."
              value={table.getState().globalFilter || ""}
